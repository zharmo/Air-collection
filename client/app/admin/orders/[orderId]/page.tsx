...
const StatusPill = ({ status }: { status: string }) => {
    const cfg = STATUS_CFG[status] || { label: status, color: '#475569', bg: '#f1f5f9', icon: null };
    return (
        <span style={{
            display: 'inline-flex', alignItems: 'center', gap: 5,
            background: cfg.bg, color: cfg.color,
            fontSize: '0.71rem', fontWeight: 700, letterSpacing: '0.05em',
...
const PayPill = ({ status }: { status: string }) => {
    const cfg = PAY_CFG[status] || { label: status, color: '#475569', bg: '#f1f5f9' };
    return (
        <span style={{
...
const CardHeader = ({ icon, title }: { icon: React.ReactNode; title: string }) => (
    <div style={{
        ...
        padding: '14px 20px', borderBottom: '1px solid #f0f3f6',
        background: '#fafbfc',
    }}>
        <span style={{ color: '#94a3b8', fontSize: 13 }}>{icon}</span>
        <span style={{ fontSize: '0.8rem', fontWeight: 650, color: '#374151', letterSpacing: '0.02em' }}>
            {title}
        </span>
    </div>
);
                                            ...
                                            <tr key={item.id}>
                                                <td>
                                                    <div className="od-product-cell">
                                                        ...
                                                        <span className="od-product-name">{item.product_name}</span>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="od-variant-cell">
                                                        {item.size  && <span className="od-variant-tag">{item.size}</span>}
                                                        {item.color && <span className="od-variant-tag">{item.color}</span>}
                                                ...
                                                <td style={{ textAlign: 'right' }}>
                                                    <span style={{ fontWeight: 650, color: '#1e293b', fontSize: '0.9rem' }}>
